...
public:
  ...
  constexpr explicit string_hash(hash_type value)
    : m_value(value) {
  }

  constexpr string_hash(const char* str, std::size_t len)
    : m_value(hasher::hash(str, len)) {
  }
  constexpr string_hash(const char* str)
    : m_value(hasher::string_hash(str)) {
  }

  inline string_hash(const std::string_view& rhs)
    : m_value(hasher::hash(rhs.data(), rhs.size())) {
  }
  inline string_hash(const std::string& rhs)
    : m_value(hasher::hash(rhs.data(), rhs.size())) {
  }

  // Literal constructors
  template <std::size_t N>
  constexpr string_hash(char (&s)[N])
    : m_value(hasher::hash(s, N - 1)) {
  }
  template <std::size_t N>
...
template <typename H>
struct hash<grlx::rpc::detail::string_hash<H>> {
  std::size_t operator()(const grlx::rpc::detail::string_hash<H>& sh) const {
